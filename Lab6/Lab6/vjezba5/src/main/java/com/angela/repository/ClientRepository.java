...
public class ClientRepository {
		...
		for (Client client : allClients) {
			if (client.getClientID() == clientID) {
				if (client.getSmartDevice() != null)
					return client.getSmartDevice().getHistory();//gson.toJson(client.getSmartDevice().getHistory());
			}
		}
	...
	public ElMeter registerNewDeviceForClient(int clientID) {
		...
		for (Client client : allClients) {
			if (client.getClientID() == clientID) {
				if (client.getSmartDevice() == null) {
					...
					throw new DataAlreadyExistsException("Client already has deviceTestError");
				}
			}
		}
		...
		return smartDevice;// gson.toJson(smartDevice);
	}

	public ArrayList<Client> getAllClients() {
		return allClients;// gson.toJson(allClients);
	}
